...
#         d_frame_b = gpuarray.zeros_like(d_frame_a)
        
        frame_a[:]=frame_b
        d_frame_b=gpuarray.to_gpu(frame_a.astype(np.float32))
#         drv.memcpy_htod(d_frame_b,frame_a)
#         d_frame_b = gpuarray.to_gpu(frame_b.astype(np.float32))
...
#         self._IWarrange(d_frame_a, d_frame_b, d_winA, d_search_area, d_shift)
        ...
        d_shift_empty = gpuarray.zeros_like(d_shift)
        self._IWarrange(d_frame_a, d_winA, d_shift_empty, "A")
        self._IWarrange(d_frame_b, d_search_area, d_shift, "B")
...
#         plt.colorbar(im1,ax=ax[1],fraction=0.04)
        plt.title("frameb",fontsize=20)
        base = "/home/xbao/LAB_test/PLUME-3_51V-B25_2-11032008/ANALYSIS/DEBUG/"
        plt.savefig(base+"frameb_"+str(time.time())+".jpg")
...
#         self._IWarrange(d_frame_b, d_search_area, d_shift, "B")
    ...
    def _IWarrange(self, d_frame, d_win, d_shift, label):
        """
            ...
            __global__ void windowSlice_shift(float *input, float *output, int *dx, int *dy, int window_size, int overlap, int n_col, int w, int h, int batch_size)
        {
            ...
            const int IW_size = window_size*window_size;
            const int ind_x = blockIdx.x*blockDim.x + threadIdx.x;
            const int ind_y = blockIdx.y*blockDim.y + threadIdx.y;
            const int diff = window_size - overlap;
            
            if (batch_size <200)
            for(int j=0;j<2;j++)
                        printf("input[%d]=%.6f,address=%p\\n",j,input[j],&input[j]);
...
#         frame=d_frame.get()
#         d_frame = gpuarray.to_gpu(frame)
#         print "window address:",label,d_frame.ptr
        windowSlice_shift = mod_ws.get_function("windowSlice_shift")
        windowSlice_shift(d_frame, d_win, d_dx, d_dy, self.window_size, self.overlap, self.n_cols, w, h, self.batch_size, block = (block_size,block_size,1), grid=(grid_size,grid_size) )
        ...
        lenA=len(win)
        for axi,indexc in zip(ax.flatten(),[0,lenA/3,lenA/2,lenA-1]):
            im=axi.imshow(win[indexc],cmap=plt.cm.gray)
            plt.colorbar(im,ax=axi,fraction=0.04)
#         im1=ax[1].imshow(frame_b,cmap=plt.cm.gray)
#         plt.colorbar(im1,ax=ax[1],fraction=0.04)
        fig.suptitle("afterwin",fontsize=20)
        base = "/home/xbao/LAB_test/PLUME-3_51V-B25_2-11032008/ANALYSIS/DEBUG/"
        plt.savefig(base+"afterwin_"+label+" "+str(time.time())+".jpg")
    ...
    def _IWarrange_old(self, d_frame_a, d_frame_b, d_winA, d_search_area, d_shift):
        """
        Creates a 3D array stack of all of the interrogation windows. 
        This is necessary to do the FFTs all at once on the GPU.
            ...
            __global__ void windowSlice(float *input, float *output, int window_size, int overlap, int n_col, int w, int batch_size)
        {
            ...
            {   
                ...
                //if (batch_size==143 & ind_x<30 & ind_y ==0)
                //if (batch_size==143 & f_range<100)
                //    printf("i= %d,f_range= %d,w_range= %d,input[f_range]= %.6f\\n",i,f_range,w_range,input[f_range]);
                if (batch_size<200 & f_range<100){
...
#         d_frame_bb=d_frame_b.copy()
#         d_frame_bb.strides = d_frame_b.strides
#         d_frame_bb.flags.f_contiguous = d_frame_b.flags.f_contiguous
#         d_frame_bb.flags.c_contiguous = d_frame_b.flags.c_contiguous
#         d_frame_bb.flags.forc = d_frame_b.flags.forc
...
#         fig,ax = plt.subplots(2,2,figsize=(20,20))
#         lenB=len(winB)
#         for axi,indexc in zip(ax.flatten(),[0,lenB/3,lenB/2,lenB-1]):
...
#     #         im1=ax[1].imshow(frame_b,cmap=plt.cm.gray)
#     #         plt.colorbar(im1,ax=ax[1],fraction=0.04)
#             fig.suptitle("shiftafterwinB",fontsize=20)
#             base = "/home/xbao/LAB_test/PLUME-3_51V-B25_2-11032008/ANALYSIS/DEBUG/"
#             plt.savefig(base+"shiftafterwinB_"+str(time.time())+".jpg")
